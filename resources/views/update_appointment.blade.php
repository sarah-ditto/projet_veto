...
<form method="POST" action="{{route('updateAppointment.post',['IDClient'=>$consult->IDClient,'IDAnimal'=>$consult->IDAnimal,'IDConsult'=>$consult->IDConsult])}}">
  ...
  <div class="form-group">
    ...
    <select id="motifConsult" name="motifConsult" aria-describedby="motifConsult_feedback" class="form-control @error('motifConsult') is-invalid @enderror" required>
  ...
  <div class="form-group">
    ...
    <textarea type="number" id="obsConsult" name="obsConsult" aria-describedby="obsConsult_feedback" class="form-control @error('obsConsult') is-invalid @enderror" maxlength=200>{{$consult->ObsConsult}}</textarea>
    @error('obsConsult')
